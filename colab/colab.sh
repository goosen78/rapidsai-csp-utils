...
install_RAPIDS () {
    ...
    if [ ! -f Miniconda3-4.5.4-Linux-x86_64.sh ]; then
        ...
        # install RAPIDS packages
            ...
            # install RAPIDS packages
            conda install -y --prefix /usr/local \
                -c rapidsai -c nvidia -c numba -c conda-forge -c defaults \
                python=3.6 gdal=3.0.4 cudatoolkit=$CTK_VERSION \
                cudf=$RAPIDS_VERSION cuml cugraph cuspatial gcsfs pynvml xgboost=1.1.0dev.rapidsai$RAPIDS_VERSION \
                dask-cudf cusignal
            conda install -y --prefix /usr/local \
                -c rapidsai -c nvidia -c conda-forge -c defaults \
                python=3.6 gdal cudatoolkit=10.1 rapids=0.14 \
                gcsfs pynvml dask-cudf python-graphviz custreamz \
                networkx nvtx pickle5 setuptools pyyaml \
                ruamel.yaml cloudpickle bqplot
        fi
