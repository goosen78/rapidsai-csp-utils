...
install_RAPIDS () {
    ...
    python /content/rapidsai-csp-utils/colab/env-check.py
    ...
    if [ ! -f ac.sh ]; then
        ...
        chmod +x ac.sh
        bash ./ac.sh -b -f -p /drive/colab-ssh

        #pin python3.6
        echo "python 3.7.*" > /drive/colab-ssh/conda-meta/pinned
        
        cd /drive/colab-ssh
        PYTHONPATH=/drive/colab-ssh/bin/python
        export PYTHONPATH=/drive/colab-ssh/bin/python
        source /drive/colab-ssh/bin/activate root
        ...
        conda install -y --prefix /drive/colab-ssh -c conda-forge -c defaults openssl six
        
        ln -s /usr/local/lib/python3.6/dist-packages/google \
        /drive/colab-ssh/lib/python3.7/site-packages/google
        ...
        # install RAPIDS packages
            ...
            echo "Please standby, this will take a few minutes..."
            # install RAPIDS packages
            conda env update --prefix /drive/colab-ssh --file ENV.yml  --prune
            source /drive/colab-ssh/bin/activate root
            ...
            cd /drive/colab-ssh/NeMo
            cp /content/nemo.patch /drive/colab-ssh/NeMo/
        ...
        cp /drive/colab-ssh/lib/libcudf.so /usr/lib/libcudf.so
        cp /drive/colab-ssh/lib/librmm.so /usr/lib/librmm.so
        cp /drive/colab-ssh/lib/libnccl.so /usr/lib/libnccl.so
        echo "Copying RAPIDS compatible xgboost"	
        cp /drive/colab-ssh/lib/libxgboost.so /usr/lib/libxgboost.so
    fi
